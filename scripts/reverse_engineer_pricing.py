...
데이터 소스: ERP DB 직접 조회 (PRT_ESTH + PRT_ESTL + MAS_EMP)
...
try:
    ...
    print("❌ pymssql 패키지가 필요합니다: pip3 install pymssql")
    sys.exit(1)
...
# 견적 데이터를 ERP DB에서 직접 조회하는 SQL입니다.
# PRT_ESTH(견적 헤더) + PRT_ESTL(견적 라인) + MAS_EMP(사원) JOIN
ERP_QUERY = """
SELECT
    h.NO_EST       AS [견적번호],
    h.DT_EST       AS [견적일],
    ...
    l.FG_EST       AS [구분],
    l.CD_ITEM      AS [항목],
    l.NM_ITEM      AS [항목명_ERP],
    l.DC_ITEM_SPEC AS [규격],
    l.QT_DASU      AS [대수],
    l.QT           AS [수량(R)],
    l.UM           AS [단가],
    l.AM           AS [금액],
    l.RT_DISCOUNT  AS [할인율],
    l.AM_SUPPLY    AS [최종금액],
    l.DC_RMK       AS [비고]
FROM PRT_ESTL l
JOIN PRT_ESTH h
...
LEFT JOIN MAS_EMP e
    ON h.CD_EMP = e.CD_EMP
WHERE h.DT_EST >= '20250601'
"""


def load_and_prepare():
    """ERP DB에서 직접 조회하고 분석용으로 준비합니다."""
    print("📂 ERP DB 접속 중...")
    conn = get_erp_connection()
    print("   ✅ 접속 성공")

    print("📥 견적 데이터 조회 중 (2025-06-01 이후)...")
    df = pd.read_sql(ERP_QUERY, conn)
    conn.close()
    print(f"   → {len(df):,}행 로드 (ERP 실시간)")

    # 항목 코드 정규화: 앞자리 '0' 제거 (0703002 → 703002)
    df["항목"] = df["항목"].astype(str).str.strip().str.lstrip("0")
    df.loc[df["항목"].isin(["nan", "None", ""]), "항목"] = pd.NA

    # 영업담당 있고 + 항목 있고 + 단가 > 0인 행만 필터
    mask = (
        df["영업담당"].notna()
        & df["항목"].notna()
        & df["단가"].notna()
...
def save_period(scorecards, fairness, anomalies, volume, client_pricing, filtered, period_key, period_label):
    ...
    dates = filtered["견적일"].dropna().astype(str)
    dates = dates[dates.str.len() == 8]
    date_from = f"{dates.min()[:4]}.{dates.min()[4:6]}" if len(dates) > 0 else "-"
    date_to = f"{dates.max()[:4]}.{dates.max()[4:6]}" if len(dates) > 0 else "-"

    output = {
        "_meta": {
            "label": "초안(ERP 역산) — 영업자 확인 전. 실제 단가표와 대조 필요.",
            "source": "ERP DB 직접 조회 (PRT_ESTH + PRT_ESTL + MAS_EMP)",
